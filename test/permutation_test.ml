...
let%expect_test "sorted_permutations_iter" =
  let p =
    Sorted.create 4 (fun x y ->
        match (x, y) with 1, 3 | 2, 3 | 2, 4 -> true | _ -> false)
  in
  Sorted.iter p ~f:(fun (c, _) -> print_s @@ [%sexp_of: Int_array.t] c);
  [%expect
    {|
    (1 2 3 4)
    (1 2 4 3)
...
let%expect_test "sorted_permutations_young" =
  let young_tableaux x y =
    ...
    | _ -> false
  in
  let p = Sorted.create 9 young_tableaux in
  Sorted.iter p ~f:(fun (_, c) -> print_s @@ [%sexp_of: Int_array.t] c);
  [%expect
    {|
    (1 2 3 4 5 6 7 8 9)
    (1 2 3 4 5 7 6 8 9)
...
let%expect_test "restricted_permutations_iter" =
  let f a =
    ...
    | _ -> true
  in
  let p = Restricted.create 4 f in
  Restricted.iter p ~f:(fun c -> print_s @@ [%sexp_of: Int_array.t] c);
  [%expect
    {|
    (1 2 3 4)
    (1 2 4 3)
